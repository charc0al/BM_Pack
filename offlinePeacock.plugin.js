...
const {
    OfficialServerAuth,
} = require("@peacockproject/core/officialServerAuth")
...
const FALLBACK_H1_EPIC_ENTITLEMENTS = [
...
module.exports = async function offlineAuthEntitlementsPlugin() {
    const epicNamespaces = [
        platformEntitlements.EPIC_NAMESPACE_2016,
        platformEntitlements.EPIC_NAMESPACE_2021,
    ].filter(Boolean)
    ...
    const h1Epic =
        platformEntitlements.H1_EPIC_ENTITLEMENTS ||
        FALLBACK_H1_EPIC_ENTITLEMENTS
    const h3Epic =
        platformEntitlements.H3_EPIC_ENTITLEMENTS ||
        FALLBACK_H3_EPIC_ENTITLEMENTS
    const h1Steam =
        platformEntitlements.H1_STEAM_ENTITLEMENTS ||
        FALLBACK_H1_STEAM_ENTITLEMENTS
    const h2Steam =
        platformEntitlements.H2_STEAM_ENTITLEMENTS ||
        FALLBACK_H2_STEAM_ENTITLEMENTS
    const h3Steam =
        platformEntitlements.H3_STEAM_ENTITLEMENTS ||
        FALLBACK_H3_STEAM_ENTITLEMENTS

    const allEpic = unique(
        (platformEntitlements.ALL_EPIC_ENTITLEMENTS || [])
            .concat(epicNamespaces)
            .concat(h1Epic)
            .concat(h3Epic),
    )
    ...
    entitlementStrategies.SteamH1Strategy.prototype.get = function () {
        return unique(
            [platformEntitlements.STEAM_NAMESPACE_2016].concat(h1Steam),
        )
    }

    entitlementStrategies.SteamH2Strategy.prototype.get = function () {
        return unique(
            [platformEntitlements.STEAM_NAMESPACE_2018].concat(h2Steam),
        )
    }
